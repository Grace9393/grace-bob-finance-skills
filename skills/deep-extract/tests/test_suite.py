...
# Fixture documents are client material and are deliberately not committed.
# Point these at local files before running the suite:
#   set DEEP_EXTRACT_TEST_MATERIALS=<folder holding the two fixtures>
#   set DEEP_EXTRACT_TEST_HEALTHY=<a .docx that opens cleanly>
#   set DEEP_EXTRACT_TEST_DAMAGED=<a .docx with a damaged container>
MATERIALS = os.environ.get("DEEP_EXTRACT_TEST_MATERIALS", "")
HEALTHY = os.path.join(MATERIALS, os.environ.get("DEEP_EXTRACT_TEST_HEALTHY", "healthy.docx"))
DAMAGED = os.path.join(MATERIALS, os.environ.get("DEEP_EXTRACT_TEST_DAMAGED", "damaged.docx"))

if not MATERIALS or not os.path.isfile(HEALTHY) or not os.path.isfile(DAMAGED):
    sys.exit(
        "test fixtures not found.\n"
        "Set DEEP_EXTRACT_TEST_MATERIALS (and optionally _HEALTHY / _DAMAGED)\n"
        "to local documents before running this suite."
    )
